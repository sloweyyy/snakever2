...
async function gameLoop(timestamp) {
  ...
  const newState = currentGame
    ...
    : await gameClient.updateState(currentRoomId);
  if (newState) {
    draw(newState);
  }
  animationHandle = requestAnimationFrame(gameLoop);
...
const presentation = {
  resizeGameArea: function () {
    ...
    const height =
      main.clientHeight < width
        ? main.clientHeight - footer.offsetHeight - 10
        : tv.clientHeight;
    screen.style.width = `${width}px`;
    screen.style.height = `${height}px`;
    canvas.width =
      screen.clientWidth % 20 == 0
        ? screen.clientWidth
        : screen.clientWidth - 20 - (screen.clientWidth % 20);
    canvas.height =
      screen.clientHeight % 20 == 0
        ? screen.clientHeight
        : screen.clientHeight - 60 - (screen.clientHeight % 20);
  },
  ...
  joinRoom: async function () {
    ...
    leaveRoom.disabled = false;
    roomIdToJoinInput.disabled = true;
    const joinRoomResponse = await gameClient.joinRoom(
